...
const LowStockAlerts: React.FC<LowStockAlertsProps> = ({
  ...
  const [outOfStockProducts, setOutOfStockProducts] = useState<
    LowStockProduct[]
  >([]);
  const [productsWithLowStockVariants, setProductsWithLowStockVariants] =
    useState<ProductWithVariants[]>([]);
  const [productsWithOutOfStockVariants, setProductsWithOutOfStockVariants] =
    useState<ProductWithVariants[]>([]);
    ...
    async (page = 1) => {
      ...
      try {
        const [alertsResponse, statsResponse] = await Promise.all([
          getLowStockAlerts({ page: 1, limit: 1000 }, accessToken),
        ...
        if (alertsResponse.success && alertsResponse.data) {
          ...
          setProductsWithLowStockVariants(
            alertsResponse.data.productsWithLowStockVariants || []
          );
          setProductsWithOutOfStockVariants(
            alertsResponse.data.productsWithOutOfStockVariants || []
          );
    ...
    return (
      <div className="flex justify-center items-center py-8">
        ...
        <span className="ml-2 text-gray-600">
          Düşük stok uyarıları yükleniyor...
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray rounded-lg shadow border">
        ...
        productsWithLowStockVariants.length > 0 ||
        productsWithOutOfStockVariants.length > 0 ? (
          ...
          <div className="px-6 py-8 text-center text-gray-500">
            ...
            <p className="text-sm text-gray-400 mt-2">
              Stok değeri 0 olan veya düşük stok eşiği altındaki ürünler burada
              görünecektir
            </p>
          </div>
        )}
